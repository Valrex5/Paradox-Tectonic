module Compiler
    ...
    def compile_pokemon
        ...
        # Read from PBS file
        File.open("PBS/pokemon.txt", "rb") do |f|
        ...
        FileLineData.linereport)
            ...
            path)
                ...
                species_hash = {
                  ...
                  :line_moves => contents["EggMoves"] || base_data.line_moves.clone,
...
FileLineData.linereport)
    ...
    def write_pokemon
        File.open("PBS/pokemon.txt", "wb") do |f|
            ...
            GameData::Species.each do |species|
				...
				uniqueTutorMoves = species.tutor_moves - species.inherited_moves
				if uniqueTutorMoves.length > 0
					f.write(format("# Moves not inherited by evolutions\r\n"))
					f.write(format("TutorMoves = %s\r\n", uniqueTutorMoves.join(",")))
				end
                uniqueLineMoves = species.line_moves - species.inherited_moves
				if uniqueLineMoves.length > 0
					f.write(format("# Moves inherited by evolutions\r\n"))
					f.write(format("LineMoves = %s\r\n", uniqueLineMoves.join(",")))
				end
    ...
    species.real_form_name))
                ...
                f.write(format("BattlerPlayerX = %d\r\n", species.back_sprite_x))
                f.write(format("BattlerPlayerY = %d\r\n", species.back_sprite_y))
                f.write(format("BattlerEnemyX = %d\r\n", species.front_sprite_x))
                f.write(format("BattlerEnemyY = %d\r\n", species.front_sprite_y))
                if species.front_sprite_altitude != 0
                    f.write(format("BattlerAltitude = %d\r\n",
    species.front_sprite_altitude))
                end
                f.write(format("BattlerShadowX = %d\r\n", species.shadow_x))
                f.write(format("BattlerShadowSize = %d\r\n", species.shadow_size))
...
species.catch_rate))
                ...
                if species.hatch_steps != base_species.hatch_steps
    ...
    species.hatch_steps))
                ...
                if species.real_category != base_species.real_category
                    f.write(format("Kind = %s\r\n",
      species.real_category))
                end
    ...
    species.real_pokedex_entry))
                ...
                if species.generation != base_species.generation
...
species.generation))
                ...
                if species.wild_item_common != base_species.wild_item_common ||
                   ...
                   species.wild_item_rare != base_species.wild_item_rare
                    f.write(format("WildItemCommon = %s\r\n", species.wild_item_common)) if species.wild_item_common
                    if species.wild_item_uncommon
                        f.write(format("WildItemUncommon = %s\r\n",
species.wild_item_uncommon))
                    end
                    f.write(format("WildItemRare = %s\r\n", species.wild_item_rare)) if species.wild_item_rare
                end
                if species.back_sprite_x != base_species.back_sprite_x
                    f.write(format("BattlerPlayerX = %d\r\n",
    species.back_sprite_x))
                end
                if species.back_sprite_y != base_species.back_sprite_y
                    f.write(format("BattlerPlayerY = %d\r\n",
    species.back_sprite_y))
                end
                if species.front_sprite_x != base_species.front_sprite_x
                    f.write(format("BattlerEnemyX = %d\r\n",
    species.front_sprite_x))
                end
                if species.front_sprite_y != base_species.front_sprite_y
                    f.write(format("BattlerEnemyY = %d\r\n",
    species.front_sprite_y))
                end
                if species.front_sprite_altitude != base_species.front_sprite_altitude
                    f.write(format("BattlerAltitude = %d\r\n",
    species.front_sprite_altitude))
                end
                if species.shadow_x != base_species.shadow_x
                    f.write(format("BattlerShadowX = %d\r\n",
species.shadow_x))
                end
                if species.shadow_size != base_species.shadow_size
                    f.write(format("BattlerShadowSize = %d\r\n",
    species.shadow_size))
                end
                next unless species.evolutions != base_species.evolutions && species.evolutions.any? { |evo| !evo[3] }
                f.write("Evolutions = ")
                need_comma = false
                species.evolutions.each do |evo|
                    next if evo[3] # Skip prevolution entries
    ...
    class Species
        ...
        def apply_metrics_to_sprite(sprite, index, shadow = false)
            if shadow
                ...
                sprite.y += @back_sprite_y * 2 # Player's Pokémon
            else # Foe Pokémon
		...
		def inherited_moves
			...
			# Add all line moves from all prevos
            speciesLookingAt = self
            while GameData::Species.get(speciesLookingAt.get_previous_species) != speciesLookingAt
                speciesLookingAt = GameData::Species.get(speciesLookingAt.get_previous_species)
                inherited_moves.concat(speciesLookingAt.line_moves)
            end
        ...
        def learnable_moves
			...
			learnableMoves.concat(learnable_moves_non_level)

            # Add level-up-learnset moves
		...
		def learnable_moves_non_level
			learnableMoves = []

			# Add specified tutor moves
            learnableMoves.concat(@tutor_moves)

			# Add our own line moves
            learnableMoves.concat(@line_moves)

			# Add moves from inheritance
            learnableMoves.concat(inherited_moves)

            # Add form-based moves
            learnableMoves.concat(form_specific_moves)

			return learnableMoves
		end

		def moves_not_inherited
			return learnable_moves - inherited_moves
		end
